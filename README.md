...
[![WordPress](https://img.shields.io/badge/WordPress-%3E%3D6.9-blue)](https://wordpress.org/)
...
- **WordPress** 6.9+ (needed for `WP_Upgrader::install_package()` automatic rollback — failed updates restore the previous plugin version from `wp-content/upgrade-temp-backup/plugins/{slug}/`)
- **PHP** 8.3+
- **Composer** 2.0+

Consumer plugins should declare `Requires at least: 6.9` in their plugin headers so older WordPress sites block activation cleanly.
